...
TrueConf Server обычно разворачивается on-prem с сертификатом от внутреннего CA или self-signed. Канал поддерживает четыре пути доверия этому сертификату, в порядке предпочтения для production:

### 1. Публичный cert (Let's Encrypt и т.п.)

Ничего делать не надо. Wizard видит cert в системном trust store и пропускает TOFU.

### 2. Enterprise CA через system trust (рекомендуется для prod)
...
Wizard валидирует файл строго (парсит как PEM + проверяет что файл реально валидирует живой сервер) до продолжения. При ошибке setup прерывается с конкретной причиной.

### 4. Interactive wizard TOFU (для быстрой локальной настройки)

Если ничего из выше не задано, wizard показывает детали сертификата сервера (владелец / издатель / срок / SHA-256 отпечаток) и просит подтвердить. **Сверьте отпечаток с админом сервера по отдельному каналу** до accept'а:
...
Wizard сохраняет цепь в `~/.openclaw/trueconf-ca.pem` (`0600`).

### Повторный запуск wizard'а

На каждом последующем `openclaw plugins setup trueconf` сохранённая цепь заново валидируется против живого сервера. Три исхода:
...
`runHeadlessFinalize` (когда CLI вызывается неинтерактивно с env-переменными `TRUECONF_SERVER_URL`/`_USERNAME`/`_PASSWORD`) повторяет логику wizard'а, но все failure-case'ы fatal — нет prompt'ов и recovery. Опции для headless TLS:
