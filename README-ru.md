...
> [!CAUTION]
> Данный плагин является gateway-интерфейсом для взаимодействия с OpenClaw. TrueConf не призывает к использованию OpenClaw, не рекомендует его внедрение в корпоративной инфраструктуре и не несёт ответственности за работу OpenClaw, связанных ИИ-агентов, внешних сервисов или сторонних компонентов.
>
> Использование плагина и подключение к OpenClaw выполняются пользователем самостоятельно и на собственный риск. Перед развертыванием в корпоративной сети необходимо ознакомиться с рисками информационной безопасности, включая утечку данных, несанкционированный доступ, prompt injection, выполнение нежелательных команд, компрометацию API-ключей и непредвиденное поведение ИИ-агентов:
>
> - [ИИ-агент или угроза? Хроника (не)безопасности OpenClaw](https://xakep.ru/2026/05/05/openclaw-exploits/)
> - [Угрозы OpenClaw: оценка рисков и реагирование на «теневой ИИ»](https://www.kaspersky.ru/blog/moltbot-enterprise-risk-management/41329/)
>
> Рекомендуется использовать данный плагин только после оценки рисков, тестирования в изолированной среде, ограничения прав доступа, настройки мониторинга и принятия мер для защиты корпоративной инфраструктуры.

> [!IMPORTANT]
> Проект всё ещё находится в разработке. Тем не менее мы будем признательны, если вы попробуете его в работе. Если вы столкнётесь с какими-либо проблемами, пожалуйста, создайте [issue](https://github.com/TrueConf/trueconf-openclaw-channel/issues) с подробным описанием.

## Как это работает?

После установки ИИ-агент OpenClaw общается с пользователями через TrueConf — сообщения и файлы отправленные боту, передаются ИИ-агенту, а ответы приходят обратно в чат.
...
|-----------|------------|---------------------|--------|
| Рендеринг текста по умолчанию | Markdown агента конвертируется в HTML TrueConf (`parseMode: 'html'`): жирный, курсив, зачёркнутый, ссылки, заголовки, списки и таблицы; переносы строк уходят как `<br>` | `ParseMode.TEXT` | LLM по умолчанию выдают markdown. HTML-режим TrueConf отображает только `<b>`, `<i>`, `<u>`, `<s>`, `<a>` и склеивает обычные переводы строк. |
...
- **Причина:** версии плагина до 1.2.9 удаляли `plugins.entries.trueconf` из `openclaw.json` как «устаревшую» запись (1.2.8 починил это для установки из tarball, но не для установки с registry — та живёт в `~/.openclaw/npm/projects/`). На openclaw 2026.6.x эта запись — живой признак включения установленного плагина, и её удаление отключало плагин.
- **Решение:** обновите плагин до >= 1.2.9 (теперь очистка срабатывает только при отсутствии следов установки), затем восстановите запись — повторно выполните `openclaw plugins install @trueconf-community/trueconf-openclaw-channel` или верните её вручную в `~/.openclaw/openclaw.json`:
  ```json
