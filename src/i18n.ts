...
const TRANSLATIONS = {
  ...
  'wizard.input.serverUrl.message':    { en: 'Server address (host or IP, without http(s)://)', ru: 'Адрес сервера (хост или IP, без http(s)://)' },
  'wizard.input.serverUrl.invalidScheme': { en: 'Remove the http(s):// prefix — only host or IP is needed', ru: 'Уберите префикс http(s):// — нужен только хост или IP' },
  'wizard.input.serverUrl.invalidPort':   { en: 'Remove the :port suffix — port is configured separately', ru: 'Уберите :порт — порт настраивается отдельно' },
