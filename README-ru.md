...
Если у TrueConf Server самоподписанный сертификат, мастер настройки предлагает три явных пути доверия по убыванию безопасности:

1. **Указать путь к CA от админа** — рекомендуемый путь. Если администратор TrueConf Server выдал вам корневой CA в формате PEM, впишите `"caPath": "/path/to/server-ca.pem"` в конфиг (или выберите «Указать путь к корневому сертификату (CA)» в интерактивном мастере).
   - На TrueConf Server сертификат обычно лежит как `*.crt`. Если файл в формате PEM, его можно переименовать в `*.pem`.
   - Если вы владелец сервера, ищите сертификат в панели управления TrueConf Server в разделе HTTPS.
2. **`NODE_EXTRA_CA_CERTS` / системный trust store** — добавить корневой CA в OS или Node-уровень (MDM, `update-ca-certificates`, или `export NODE_EXTRA_CA_CERTS=/etc/ssl/certs/corp-ca.pem`). Probe увидит cert как доверенный, и `caPath` вообще не потребуется.
3. **Отключить проверку TLS** (`tlsVerify: false`) — последнее средство. Отключает проверку сертификата **только для этого TrueConf Server** (другие HTTPS-вызовы из Node остаются проверяемыми). Выберите «Отключить проверку TLS-сертификата для этого TrueConf Server» в мастере, или впишите `"tlsVerify": false` в конфиг, или передайте `TRUECONF_TLS_VERIFY=false` в headless-настройку.

> **Безопасность** Вариант #3 открывает TrueConf-трафик для MITM-атаки — используйте его только в безопасной или ограниченной среде (offline-лаб, доверенная внутренняя сеть). Вариант #1 ближе всего к production-grade.

> **Чего не стоит делать.** Не ставьте `NODE_TLS_REJECT_UNAUTHORIZED=0`. Эта переменная отключает проверку TLS для **всего** Node-процесса — каждый LLM-провайдер, каждый webhook, каждый HTTPS-запрос. Опция `tlsVerify: false` выше — per-TrueConf only.
