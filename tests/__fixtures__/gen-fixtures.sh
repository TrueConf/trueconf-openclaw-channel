...
#
# NOTE for secret scanners / audits: the .key files in this directory are
# throwaway RSA keys generated by this script. They sign ONLY the self-signed
# test CAs (ca-valid, ca-other) that are bound to 127.0.0.1 / localhost and
# used exclusively by the local vitest TLS fixtures. They do not authenticate
# anything in production and carry no access to any real system. Safe to
# commit; they exist to keep the test suite hermetic (no openssl dependency
# at test run-time).
set -euo pipefail
cd "$(dirname "$0")"
