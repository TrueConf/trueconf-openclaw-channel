...
Соединение WebSocket с TrueConf Server работает в выделенном `worker_thread`
(один на каждую учётную запись). Главный поток обрабатывает рантайм OpenClaw,
вызовы LLM, обработку изображений и очередь исходящих сообщений. Такое
разделение гарантирует, что длительные операции LLM на главном потоке не
могут заблокировать heartbeat WebSocket (это была предыдущая причина
аномальных закрытий 1006).

Двусторонний watchdog: главный поток пингует worker каждые 5 с; если ответ
`appPong` не приходит за 15 с, главный поток перезапускает worker. Worker
пингует сервер TrueConf на уровне WS-протокола каждые 10 с с таймаутом
pong'а 5 с. Авто-подтверждение фреймов `type:1` выполняется внутри worker'а
за микросекунды после получения, независимо от нагрузки главного потока.

Полный дизайн — см. `docs/specs/2026-05-14-ws-worker-thread-design.md`.
